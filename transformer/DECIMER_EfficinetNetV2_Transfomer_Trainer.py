...
logger.info("Number of devices: {}".format(strategy.num_replicas_in_sync), flush=True)
logger.info("REPLICAS: ", strategy.num_replicas_in_sync, flush=True)
logger.info(datetime.now().strftime("%Y/%m/%d %H:%M:%S"), "Network Started", flush=True)
...
logger.info("Total train steps: ", TRAIN_STEPS)
...
logger.info(f"Learning rate config:\n\t -> {training_config.lr_config}\n")
...
logger.info(datetime.now().strftime("%Y/%m/%d %H:%M:%S"), "Network Completed", flush=True)
f.close()
