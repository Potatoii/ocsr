...
logger.info(datetime.now().strftime("%Y/%m/%d %H:%M:%S"), "Process started", flush=True)
...
def main():
    ...
    logger.info("Tokens: ", tokenizer, flush=True)
    logger.info("Max length of attention weights: ", max_length, flush=True)
    logger.info("Total train files:", len(img_name_vector), flush=True)
    ...
    for i in range(int(len(img_name_vector) / end)):
        ...
        logger.info("Total Train_Images: ", len(imgs_path), flush=True)
        logger.info("Total SELFIES_Images: ", len(caps_path), flush=True)

        start = start + len(imgs_path)
        start_x = start_x + 1

    logger.info(datetime.now().strftime("%Y/%m/%d %H:%M:%S"), "Process completed", flush=True)
...
def data_loader(Smiles_Path):
    ...
    logger.info("Total number of selected SMILES Strings: ", len(train_captions), flush=True)
